...
spec = json.load(open(
    r"C:\quantum ai 2026\VW_SUBMISSION_PACKAGE\results\vw_jam_relaxation_spectrum_v2_shim.json"))
SQ = spec["spectra"]["0.5"]
...
def main():
    ...
    json.dump(dict(card="CERT-GAP scored sweep: certified boundary under the "
                   ...
                   source_spectrum="vw_jam_relaxation_spectrum_v2_shim.json "
                        "(fez d9rm4j9dsedc73agrb70)",
                   seeds=[21, 22, 23], episodes_per_cell=N,
                   rows=allrows),
              open(r"C:\quantum ai 2026\vw_cert_gap_score_v2.json", "w"),
              indent=1)
    return 0
